...
server.registerTool(
  ...
  {
    description:
      "Safe HTTP call to the dev API: GET under /api/v1/ only, or POST only for /api/v1/auth/login and /api/v1/auth/refresh. Host must be localhost/127.0.0.1 unless MCP_ALLOW_NON_LOCALHOST=1. Optional Bearer from CADDY_API_TOKEN. High-impact paths (apply, reload, sync, discovery run) are blocked.",
    inputSchema: {
      method: z.enum(["GET", "POST"]).describe("Only GET or POST (POST limited to auth)."),
      path: z
        .string()
