...
server.registerTool(
  ...
  {
    description:
      "Safe HTTP call to the dev API: GET under /api/v1/ only, or POST only for /api/v1/auth/login, /api/v1/auth/refresh, and /api/v1/auth/logout. Host must be localhost/127.0.0.1 unless MCP_ALLOW_NON_LOCALHOST=1. Optional Bearer from CADDY_API_TOKEN. High-impact paths (apply, reload, sync, discovery run) are blocked.",
    inputSchema: {
