...
**Upstream registration (EC2):** production automation uses `POST /api/v1/discovery/{discovery_config_id}/register-upstream` with a scoped API key (`cdk_live_…`), not the MCP. See [`scripts/ec2-register-upstream.sh`](../../scripts/ec2-register-upstream.sh) and root `README.md`.
