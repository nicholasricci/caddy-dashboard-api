...
- Solo admin JWT: **scheduled tasks** — `GET/POST /api/v1/scheduled-tasks`, `GET/PUT/DELETE /api/v1/scheduled-tasks/:id`, `POST /api/v1/scheduled-tasks/:id/toggle`, `POST /api/v1/scheduled-tasks/:id/run-now`, `GET /api/v1/scheduled-tasks/:id/logs`. Creazione/modifica/cancellazione/toggle triggerano hot-reload dello scheduler engine. Task types supportati: `discovery_run`, `token_cleanup`, `node_healthcheck`, `upstream_healthcheck`. Config obbligatori per task: `discovery_run` → `{"discovery_config_id":"<uuid>"}`; `upstream_healthcheck` → `{"discovery_config_id":"<uuid>", "config_ids":["@id",...]}` (`config_ids` opzionale, filtra gli handler Caddy).
