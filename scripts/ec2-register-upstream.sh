#!/usr/bin/env bash
# EC2 user-data helper: register this instance as a Caddy upstream on a discovery group.
set -euo pipefail

: "${CADDY_DASHBOARD_URL:?CADDY_DASHBOARD_URL is required}"
: "${DISCOVERY_CONFIG_ID:?DISCOVERY_CONFIG_ID is required}"
...
payload="$(printf '{"config_id":"%s","port":%s,"private_ip":"%s"}' "$CONFIG_ID" "$UPSTREAM_PORT" "$PRIVATE_IP")"
url="${CADDY_DASHBOARD_URL%/}/api/v1/discovery/${DISCOVERY_CONFIG_ID}/register-upstream"

attempt=1
backoff="$INITIAL_BACKOFF_SEC"
while [[ "$attempt" -le "$MAX_ATTEMPTS" ]]; do
  sleep $((RANDOM % 5))
  http_code="$(curl -sS -o /tmp/register-upstream.json -w '%{http_code}' -X POST "$url" \
    -H "Authorization: Bearer ${API_KEY}" \
    -H "Content-Type: application/json" \
    -d "$payload" || echo "000")"

  if [[ "$http_code" == "200" ]]; then
    cat /tmp/register-upstream.json
    exit 0
  fi

  if [[ "$http_code" == "409" || "$http_code" == "502" || "$http_code" == "503" || "$http_code" == "504" || "$http_code" == "000" ]]; then
    echo "register-upstream attempt ${attempt}/${MAX_ATTEMPTS} failed (HTTP ${http_code})" >&2
    [[ -f /tmp/register-upstream.json ]] && cat /tmp/register-upstream.json >&2 || true
    sleep "$backoff"
    backoff=$((backoff * 2))
    attempt=$((attempt + 1))
    continue
  ...
  echo "register-upstream failed (HTTP ${http_code})" >&2
  [[ -f /tmp/register-upstream.json ]] && cat /tmp/register-upstream.json >&2 || true
  exit 1
done

echo "register-upstream exhausted retries" >&2
exit 1
